...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: {
        id: `${siteUrl}/id${generateHref(news.slug)}`,
        en: `${siteUrl}/en${generateHref(news.slug)}`,
      },
    },
    openGraph: {
      title,
...
export default async function BeritaDetailPage({ params }: Props) {
  ...
  const title = t(`Roominformation.${news.contentKey}_title`);
  const description = t(`Roominformation.${news.contentKey}_description`);
  const date = t(`Roominformation.${news.contentKey}_date`);
  const gallery = getNewsGalleryImages(news);
  const otherNews = newsData;

  // t.raw() mengembalikan nilai asli (array atau string) dari i18n
  const rawContent = t.raw(`Roominformation.${news.contentKey}_content_1`);
